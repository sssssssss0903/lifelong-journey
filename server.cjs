...
const exportDir = isPackaged
  ? path.join(path.dirname(process.execPath), 'exports')
...
app.get('/api/export', (req, res) => {
  ...
  console.log('[导出请求]', req.query); //  打印所有请求参数

  if (!username || !/^[a-zA-Z0-9_]+$/.test(username)) {
    console.warn('[导出] 非法用户名:', username);
    return res.status(400).json({ error: '非法用户名' });
  }
  ...
  console.log('[导出] SQL:', sql, '参数:', params);

  db.query(sql, params, (err, results) => {
    if (err || !results.length) {
      console.error('[导出] 查询失败或无数据:', err);
    ...
    if (type === 'csv') {
      try {
        ...
        console.log('[导出] CSV 生成成功:', filepath);
        res.setHeader('Cache-Control', 'no-store');
        return res.download(filepath, filename);
      } catch (e) {
        console.error('[导出] CSV 生成失败:', e);
        return res.status(500).json({ error: 'CSV导出失败' });
      }
    }
    ...
    if (type === 'pdf') {
      try {
        ...
        if (!fs.existsSync(fontPath)) {
          console.error('[导出] 字体文件不存在:', fontPath);
        ...
        results.forEach((log, idx) => {
          ...
          try {
            ...
            imagePaths.forEach((url, i) => {
              ...
              console.log(`[导出] 检查图片路径:`, localPath);
              ...
              if (fs.existsSync(localPath)) {
                try {
                  ...
                  console.warn(`[导出] 第 ${i + 1} 张图片插入失败`, imgErr);
                  doc.font(fontPath).text(`⚠️ 第 ${i + 1} 张图片插入失败: ${imgErr.message}`);
                }
              } else {
                console.warn('[导出] 找不到图片:', localPath);
                doc.text(`⚠️ 找不到图片文件: ${filename}`);
              }
            });
          } catch (e) {
            console.error('[导出] 图片路径 JSON 解析失败:', e);
            doc.text('⚠️ 图片路径解析失败');
          }
        ...
        doc.end();
        stream.on('finish', () => {
          console.log('[导出] PDF 写入完成:', filepath);
          res.setHeader('Cache-Control', 'no-store');
          return res.download(filepath, filename);
        });

        stream.on('error', (e) => {
          console.error('[导出] 写入 PDF 失败:', e);
          res.status(500).json({ error: 'PDF导出失败' });
        });
      } catch (e) {
        console.error('[导出] PDF 生成流程异常:', e);
        return res.status(500).json({ error: 'PDF导出异常' });
      }
    }
  });
});

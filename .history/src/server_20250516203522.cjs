...
app.get('/api/export', (req, res) => {
  ...
  // 加载中文字体 simsunb.ttf（放在 src/fonts 目录）
  const fontPath = path.join(__dirname, 'fonts', 'simsunb.ttf');
  if (!fs.existsSync(fontPath)) {
    console.error('字体文件不存在:', fontPath);
    return res.status(500).json({ error: '找不到字体文件，请检查路径' });
  }
  doc.font(fontPath);
doc.text('测试中文字体：你好，世界');
  ...
  results.forEach((log, idx) => {
    ...
    try {
      ...
      imagePaths.forEach((url, i) => {
        const localPath = path.join(__dirname, 'uploads', path.basename(url));
        if (fs.existsSync(localPath)) {
          doc.image(localPath, { fit: [400, 300], align: 'center' });
        }
      });
    } catch (e) {
      doc.text('图片读取失败');

              ...
              {log.image_path && (
                <div style={{ marginTop: '10px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {(JSON.parse(log.image_path) || []).map((src, idx) => (
            ...
            {!showLogs ? (
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '0 10px' }}>
                ...
                <div style={{ marginBottom: '10px' }}>
                  ...
                  <button onClick={() => fetchLogs(1)}>搜索</button>
                ...
                {logDetails.map((log, idx) => (
                  <div key={idx} className="log-item">
                    ...
                    <button className="delete-button" onClick={() => handleDeleteLog(log.id)}>删除</button>
                  </div>
                ))}
                <div className="pagination">
                  <button disabled={page === 1} onClick={() => { setPage(page - 1); fetchLogs(page - 1); }}>上一页</button>
                  <span>{page} / {totalPages}</span>
                  <button disabled={page === totalPages} onClick={() => { setPage(page + 1); fetchLogs(page + 1); }}>下一页</button>
                </div>
              </div>
            )}
          </>

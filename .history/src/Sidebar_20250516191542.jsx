...
export default function Sidebar({ type = 'default', image, onClose, username, log, refreshLogs }) {
  ...
  const [selectedLog, setSelectedLog] = useState(null); // 🌟 用于切换到详情
  ...
  const fetchLogs = async (pageOverride = page) => {
    try {
      ...
      setShowLogs(true);
      setSelectedLog(null); // 返回列表时重置详情视图
    } catch (err) {
        ...
        {(type === 'logDetail' && log) || selectedLog ? (
          <>
            <button className="close-btn" onClick={() => {
              if (onClose) onClose();
              setSelectedLog(null); // 返回列表
            }}>←</button>
            <div className="panel-content">
              <h3>📍 {(selectedLog || log).location_display_name || (selectedLog || log).location_name}</h3>
              <p><strong>🕒 时间：</strong>{new Date((selectedLog || log).created_at).toLocaleString()}</p>
              <p><strong>📝 日志内容：</strong>{(selectedLog || log).content}</p>
              {(selectedLog || log).image_path && (
                <div style={{ marginTop: '10px' }}>
                  <strong>🖼 图片：</strong>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {(JSON.parse((selectedLog || log).image_path) || []).map((src, idx) => (

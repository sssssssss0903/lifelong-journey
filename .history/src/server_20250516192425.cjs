...
app.get('/api/export', (req, res) => {
  const { username, type = 'csv' } = req.query;
  if (!username || !/^[a-zA-Z0-9_]+$/.test(username)) {
    return res.status(400).json({ error: '非法用户名' });
  }

  const logTable = `${username}_log`;
  const sql = `SELECT id, location_name, location_display_name, longitude, latitude, content, image_path, created_at FROM \`${logTable}\``;
  ...
  db.query(sql, (err, results) => {
    ...
    if (type === 'csv') {
      ...
      const filename = `logs_export_safeascii.pdf`;
      const filepath = path.join(__dirname, filename);
      const stream = fs.createWriteStream(filepath);
      doc.pipe(stream);

      results.forEach((log, idx) => {
        doc.fontSize(14).text(`日志 ${idx + 1}`, { underline: true });

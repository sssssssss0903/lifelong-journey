...
app.post('/api/register', (req, res) => {
  const { username, password } = req.body;
  const createUser = 'INSERT INTO user (username, password, logs_count, medals_count) VALUES (?, ?, 0, 0)';
...
app.get('/api/user-stats', (req, res) => {
  ...
  const sql = `SELECT logs_count, medals_count FROM user WHERE username = ?`;

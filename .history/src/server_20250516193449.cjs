...
app.get('/api/export', (req, res) => {
  const { username, type = 'csv', id } = req.query;
  ...
  const sql = id ? `${baseSql} WHERE id = ?` : baseSql;
  const params = id ? [id] : [];

  db.query(sql, params, (err, results) => {
    if (err || !results.length) return res.status(500).json({ error: '查询日志失败或无数据' });

    const now = Date.now();
    const filename = id
      ? `log_${id}_${type}.${type === 'csv' ? 'csv' : 'pdf'}`

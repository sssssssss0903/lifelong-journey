...
app.post('/api/upload-log', upload.single('image'), (req, res) => {
  const { username, location_name, location_display_name, longitude, latitude, content } = req.body;
  ...
  db.query(checkLocationSQL, [location_name], (err, results) => {
    ...
    db.query(insertSQL, [location_name, location_display_name, longitude, latitude, image_path, content], (err) => {
...
app.get('/api/user-log', (req, res) => {
  ...
  const logTable = `${username}_log`;
  const sql = `SELECT location_name,long FROM \`${logTable}\``;
